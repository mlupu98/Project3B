...
#     if indirection != "":
#         print(status, indirection, 'BLOCK', block_number,
...
#                 print("INODE", int(
#                     line[1]), "HAS", references[line[1]], "LINKS BUT LINKCOUNT IS", int(line[6]))
#         else:
#             print("INODE", int(line[1]),
#                   "HAS 0 LINKS BUT LINKCOUNT IS", int(line[6]))
...
#             print("DIRECTORY INODE", line[0], "NAME",
#                   line[2], "INVALID INODE", line[1])
#         if int(line[1]) in freeInodes and allocatedOnFree == 0:
#             print("DIRECTORY INODE", line[0], "NAME",
#                   line[2], "UNALLOCATED INODE", line[1])
#         if line[2] == "'.'":
#             if line[0] != line[1]:
#                 print("DIRECTORY INODE",
#                       line[0], "NAME '.' LINK TO INODE", line[1], "SHOULD BE", line[0])
#         if line[2] == "'..'":
...
#         # appendInfo = [indirectionType[int(line[2])], line[1], line[3]]
#         # blockReference[int(line[5])].append(appendInfo)
#         # for ref in blockReference[int(line[5])]:
#         #     print("DUPLICATE", ref[0], "BLOCK", int(line[5]),
#         #           "IN INODE", ref[1], "AT OFFSET", ref[2])
#     # Add block reference.
...
#         elif (len(blockReference[blockNum]) > 1):
#             for ref in blockReference[blockNum]:
#                 if (ref[0] == ""):
#                     print("DUPLICATE BLOCK", blockNum,
#                           "IN INODE", ref[1], "AT OFFSET", ref[2])
#                 else:
#                     print("DUPLICATE", ref[0], "BLOCK", blockNum,
#                           "IN INODE", ref[1], "AT OFFSET", ref[2])
...
def checkInodes():
    ...
    for line in inodes.values():
        ...
        if (int(line[25]) != 0):
            check_block('DOUBLE INDIRECT', line[25], line[1], 256 + 12)
        if (int(line[26]) != 0):
            check_block('TRIPLE INDIRECT', int(
                line[26]), line[1], (256 * 256) + 256 + 12)
