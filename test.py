    ...
    for line in file:
        ...
        if fields[0] == 'GROUP':
            ...
            first_valid_block = int(
                fields[8]) + inode_size * num_inodes / block_size
        ...
        if fields[0] == 'INODE':
            ...
            for i in range(12, 27):
                ...
                # a 2 element structure with {'type', inode #}
    ...
    for blocknum, lst in blocks.iteritems():
        ...
        if len(lst) > 1:
            ...
            if allocated_and_free[0] == 1 and allocated_and_free[1] == 1:
                sys.stdout.write('ALLOCATED BLOCK ' +
                                 str(blocknum)+' ON FREELIST'+'\n')
                correct = False
            if num_references > 1:
                for item in lst:  # yeah, I looped over it again... maybe there's a better way to do this
                    if item[0] != 'free':
                        ...
                        inum = item[1]
                        offset = item[2]
                        sys.stdout.write('DUPLICATE '+typ+'BLOCK '+str(blocknum) +
                                         ' IN INODE '+str(inum)+' AT OFFSET '+str(offset)+'\n')
                        correct = False
        ...
        for item in lst:
            ...
            if blocknum < 0 or blocknum > num_blocks:
                if typ != '':
                    typ += ' '
                sys.stdout.write('INVALID '+typ+'BLOCK '+str(blocknum) +
                                 ' IN INODE '+str(inum)+' AT OFFSET '+str(offset)+'\n')
                correct = False
            ...
            if blocknum > 0 and blocknum < first_valid_block:
                if typ != '':
                    typ += ' '
                sys.stdout.write('RESERVED '+typ+'BLOCK '+str(blocknum) +
                                 ' IN INODE '+str(inum)+' AT OFFSET '+str(offset)+'\n')
                correct = False
...
def inodeDirCheck(file):
    ...
    for line in file:
        ...
        if fields[0] == 'INODE':
            ...
            if inodeNum in freenodes:
                sys.stdout.write('ALLOCATED INODE ' +
                                 str(inodeNum) + ' ON FREELIST'+'\n')
                correct = False
            allocnodes.add(inodeNum)
            linkCnt = int(fields[6])
            if inodeNum in linkCounts and linkCounts[inodeNum] != linkCnt:
                sys.stdout.write('INODE ' + str(inodeNum) + ' HAS ' +
                                 str(linkCounts[inodeNum]) + ' LINKS BUT LINKCOUNT IS ' + str(linkCnt) + '\n')
                correct = False
            elif inodeNum not in linkCounts:
                sys.stdout.write('INODE ' + str(inodeNum) +
                                 ' HAS 0 LINKS BUT LINKCOUNT IS ' + str(linkCnt) + '\n')
                correct = False
    ...
    for x in range(firstNode, numNodes + 1):
        if x not in freenodes and x not in allocnodes:
            sys.stdout.write('UNALLOCATED INODE ' + str(x) +
                             ' NOT ON FREELIST' + '\n')
            correct = False
    file.seek(0)
    for rawline in file:
        ...
        if fields[0] == 'DIRENT':
            ...
            if inodeNum not in allocnodes and inodeNum in range(1, numNodes + 1):
                sys.stdout.write('DIRECTORY INODE ' + str(dirNum) + ' NAME ' +
                                 fields[6] + ' UNALLOCATED INODE ' + str(inodeNum) + '\n')
                correct = False
            elif inodeNum < 1 or inodeNum > numNodes:
                sys.stdout.write('DIRECTORY INODE ' + str(dirNum) + ' NAME ' +
                                 fields[6] + ' INVALID INODE ' + str(inodeNum) + '\n')
                correct = False
            name = fields[6]
            if fields[6] == "'.'":
                if fields[3] != fields[1]:
                    sys.stdout.write('DIRECTORY INODE ' + str(dirNum) + ' NAME ' +
                                     fields[6] + ' LINK TO INODE ' + fields[3] + ' SHOULD BE ' + fields[1] + '\n')
                    correct = False
            if fields[6] == "'..'":
                if int(fields[3]) != parentInode[int(fields[1])]:
                    sys.stdout.write('DIRECTORY INODE ' + str(dirNum) + ' NAME ' +
                                     fields[6] + ' LINK TO INODE ' + fields[3] + ' SHOULD BE ' + str(parentInode[int(fields[1])]) + '\n')
                    correct = False
    return correct
